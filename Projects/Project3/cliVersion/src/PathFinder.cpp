...
void PathFinder::dijkstra(int src, std::vector<double> &dist, std::vector<int> &prev, int identityFlag, bool carMode) const {
...
static unsigned long long edgeKey(int u, int v){ return ( (unsigned long long)u << 32 ) | (unsigned long long)(unsigned int)v; }

void PathFinder::dijkstra(int src, std::vector<double> &dist, std::vector<int> &prev, int identityFlag, bool carMode, const std::unordered_set<unsigned long long> &bannedEdges, const std::unordered_set<int> &bannedNodes) const {
    ...
    while(!pq.empty()){
        ...
        // iterate neighbors and relax edges that are allowed for this identity/mode
        ...
        for (auto &e: data.graph.neighbors(u)){
            ...
            // access: 0 public, 1 FDU-only, 2 SHUTCM-only
            if (e.access == 1 && !(identityFlag & 1)) continue;
            if (e.access == 2 && !(identityFlag & 2)) continue;
            if (carMode && e.access != 0) continue; // cars cannot enter campus edges
            int v=e.to; double nd = d + e.dist;
...
std::vector<std::vector<char>> PathFinder::kShortestPaths(char s, char t, int K, int identityFlag, bool carMode) const {
    ...
    for (int k=1;k<K;++k){
        ...
        for (size_t i=0;i+1<A[k-1].size();++i){
            ...
            double td=0; for (size_t z=0; z+1<total.size(); ++z){ td += data.graph.edgeDistance(data.graph.nodeIndex(total[z]), data.graph.nodeIndex(total[z+1])); }
            ...
            for (char transferStop : data.busStops) {
                ...
                for (size_t i1=0; i1<br1.seq.size(); ++i1) {
                    ...
                    for (size_t i2=0; i2<br2.seq.size(); ++i2) {
                        ...
                        if (zeroTransfer) {
                            // merge b1 and b2 into a single bus step
                            PlanStep mergedBus;
                            mergedBus.mode = "bus";
                            mergedBus.distance = bus1dist + bus2dist;
                            mergedBus.time = mergedBus.distance / sp.bus;
                            // combine paths without duplicating transferStop
                            // b1.path currently contains sequence from stopA1 to transferStop
                            // b2.path contains sequence from transferStop to stopB2
                            for (size_t k=0;k<b1.path.size();++k) mergedBus.path.push_back(b1.path[k]);

                            // append br2.seq segment after transferStop to merged path
                            if (transferIdx2 < i2) {
                            ...
                            // combine bus numbers into a single note
                            std::string busNoteA = b1.note;
                            std::string busNoteB = std::string("bus ") + br2.busNo + std::string(" from ") + transferStopName + std::string(" to ") + stopB2Name;
                            mergedBus.note = busNoteA + std::string(" ; ") + busNoteB;
